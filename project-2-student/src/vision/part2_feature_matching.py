...
def match_features_ratio_test(
    ...
    nearest_distances = sorted_dists[:, 0]
    second_nearest_distances = sorted_dists[:, 1]

    ratios = nearest_distances / (second_nearest_distances + 1e-10)
    ratio_threshold = 0.8
    ...
    matches = np.stack([
        valid_indices,
        sorted_indices[valid_indices, 0]
    ], axis=1).astype(int)

    confidences = 1.0 - ratios[valid_indices]

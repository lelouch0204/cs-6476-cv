...
def get_gradient_histogram_vec_from_patch (
    ...
    # Divide the 16x16 patch into 4x4 cells (each cell is 4x4 pixels)
    cell_size = 4
    
    # Process each cell in reading order (left to right, top to bottom)
    for i in range(0, 16, cell_size):  # rows
        for j in range(0, 16, cell_size):  # columns
            # Extract the cell
            start_row = i
            ...
            cell_magnitudes = window_magnitudes[start_row:end_row, start_col:end_col]
            cell_orientations = window_orientations[start_row:end_row, start_col:end_col]
            
            # Flatten the cell data
            magnitudes_flat = cell_magnitudes.flatten()
            orientations_flat = cell_orientations.flatten()
            
            # Create histogram weighted by magnitudes
            hist, _ = np.histogram(orientations_flat, bins=bins, weights=magnitudes_flat)
...
def get_feat_vec(
    ...
    left_above_extension = half_feature_width - 1
    right_below_extension = half_feature_width + 1

    start_row = r - left_above_extension
    end_row = r + right_below_extension
    start_col = c - left_above_extension
    end_col = c + right_below_extension

    window_magnitudes = magnitudes[start_row:end_row, start_col:end_col]
    window_orientations = orientations[start_row:end_row, start_col:end_col]

...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    pad_height = k // 2
    pad_width = j // 2
    paddded_image = np.pad(image_bw, ((pad_height, pad_height), (pad_width, pad_width)), mode='constant')
    ...
    for i in range(m):
        for l in range(n):
            region = paddded_image[i:i+k, l:l+j]
            Ix[i, l] = np.sum(region * SOBEL_X_KERNEL)
            Iy[i, l] = np.sum(region * SOBEL_Y_KERNEL)
